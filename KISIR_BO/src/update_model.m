function [A,model] = update_model(model, f_t, final_xatminY,A,init_size)
% decide whether the model has been exploting too much.
gamma=model.gamma;


if mod(model.n,20)==0  
    model.centerX=[model.X(1:model.n,:);final_xatminY];   
end

KX = KGaussian(gamma,[model.X(1:model.n,:);final_xatminY],model.centerX);
model.KcenterX=KX;

yy=[model.f', f_t]';
A = KSIR(KX, yy, size(A,2)+1);
final_xatmin=KX*A;
...
% Book keeping.
model.n = model.n + 1;
model.X(model.n,:) = final_xatminY;
model.XT(model.n,:) = final_xatmin(model.n,:);
model.f = [model.f', f_t]';

if f_t > model.max_val
    model.max_x = final_xatminY;
    model.max_xT = final_xatmin(model.n,:);
    model.max_val = f_t;
end

if model.display % && model.n - 1 > model.last_display + 5
        fprintf('Iteration: %3i:  Max Value: %e\n', ...
            model.n-init_size, model.max_val);
        model.last_display = model.n-1;
    end

if model.n-1 >= 10 && mod(model.n-1, 20) == 0
...
model = update_kernel(model, final_xatmin(model.n,:), learn_hyper);        % Update Kernel.
A=A;
%% Update Kernel after seeing new data.
function model = update_kernel(model, x, learn_hyper)
    m = model.m;
    k_x = model.cov_model(model.hyp, model.XT(1:m,:), x);
    ...
    if ~learn_hyper
        ...
        if strcmp(model.kernel_type,'custom')
            ...
            model.L = model.cov_model(model.hyp, ...
                model.XT(1:m,:), model.XT(1:m,:)) + eye(model.m)*model.noise;
        end

        % Do Cholesky decomposition.
%         gg=model.L;
%         nn=size(gg,1);
%         [model.L,p] = chol(model.L, 'lower');
%         
%         if p>0
%            model.L(nn,nn)=0;
%         end
...
function [lml] = log_marginal_likelihood(hyper_param, model)
    ...
    if strcmp(model.kernel_type,'custom')
        ...
        % Compute kernel matrix.
        K = model.cov_model(hyper_param, model.XT(1:model.n,:), ...
            model.XT(1:model.n,:)) + eye(model.n)*model.noise;
    end

    [Kchol,p] = chol(K);
    if p>0
       lml=0.5*log(det(K))+0.5*model.f'*inv(K)*model.f;
    else
